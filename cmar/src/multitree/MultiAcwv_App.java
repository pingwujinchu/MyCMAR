...
public class MultiAcwv_App extends Classifier{
...
//			 int numClass=m_onlyClass.numDistinctValues(0);//number of classValue
		 ...
		 double[] supB = new double[numClass];
		  ...
		  //将实例依据类别分开
		  //************************************
		 Instances mul_ins = new Instances(data);
		 Instances mul_ins_arr[] = new Instances[data.attribute(data.numAttributes()-1).numValues()]; 
		 for(int i = 0 ; i < mul_ins_arr.length ; i++){
			 mul_ins_arr[i] = new Instances(mul_ins);
		 }
		 
		 for(int i = 0 ; i < data.numInstances() ; i++){
			 mul_ins_arr[(int) data.instance(i).value(data.numAttributes()-1)].add(data.instance(i));
		 }
		 
		 //*************************************
		 //使用每类实例构建树构建了多棵树
		 //*************************************
		 heads = new FastVector[numClass];
		 f = new FP[numClass];
		 for(int i = 0 ; i <numClass ; i++){
			 f[i] = new FP();
			 long t1 = System.currentTimeMillis();
			 Instances mul_data =  LabeledItemSet.divide(mul_ins_arr[i],false);
			 
			 Instances mul_only_class =  LabeledItemSet.divide(mul_ins_arr[i],true);
			 heads[i] = f[i].buildClassifyNorules(data,mul_data, mul_only_class, minSup, 1, 1, minCon,i);
...
//		dPro = newcalculatePro(l,instance);
	   if (c > 1){
		...
		for(int i = 0 ; i < l ; i++){
		  dPro[i] = f[i].calculateVote(instance, heads[i], classValue,i)[0];
		}
		long t2 = System.currentTimeMillis();
		timecost += (t2 - t1);
		System.out.println(Arrays.toString(dPro));
	   }
	    count++;
...
//		  String[] arg16={"-t","hepatiout.arff"};
		  ...
		  String[] arg17={"-t","keel/iris0.arff","-i"};
...
//		  runClassifier(new ACWV(), arg16);
		  ...
		  runClassifier(new MultiAcwv_App(), arg17);
//		  runClassifier(new ACWV(), arg18);
//		  runClassifier(new ACWV(), arg19);
//		  runClassifier(new ACWV(), arg20);
