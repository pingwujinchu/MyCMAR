...
public class ItemSet
  ...
  //新添加函数
  //*******************************
  public void addTNode(TNode tn){
	  m_items[tn.attr] = tn.value;
	  if(m_counter > tn.m_counter  || m_counter == 0){
		  m_counter = tn.m_counter;
	  }
  }
  ...
  //新添加构造函数
  //********************************
  public  ItemSet(int num,int total){
	  m_totalTransactions = total;
	  m_items = new int[num];
	  for(int i = 0 ; i < num ; i++){
		  m_items[i] = -1;
  ...
  public ItemSet(ItemSet itemset){
	  ...
	  this.m_items = new int[itemset.m_items.length];
	  System.arraycopy(itemset.m_items, 0, this.m_items, 0, itemset.m_items.length);
  }
