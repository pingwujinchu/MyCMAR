...
//      return "\nNo large itemsets and rules found!\n"+m_Ls.size();
    if(m_Rules.size() == 1){
        ...
        for (int j = 0; j < ((FastVector)m_Ls.elementAt(0)).size(); j++)
            text.append(((AprioriItemSet)((FastVector)m_Ls.elementAt(0)).elementAt(j)).
...
//    displayKsets(kSets);
//    m_text=m_text+"   "+"..."+i+" -----"+kSets.size()+"..."+m_minSupport+"\n";
...
//	rules = currentLabeledItemSet.generateRulesII(supB,m_minMetric,false);
...
//		  }
   ...
   public FastVector[] findCarLargeItemSetFPTree(Instances instances,Instances onlyClass,double min,double max,double minMetric) throws Exception{
 	  FastVector  kSets,kMinusOneSets;
 	 FastVector Iset = new FastVector();
...
//   public FastVector[] newCMAR(Instances instances,Instances onlyClass,double min,double max,double minMetric) throws Exception{
   public void newCMAR(Instances instances,Instances onlyClass,double min,double max,double minMetric) throws Exception{
...
//   private CMARtree newbuildCMARtree(Instances instances,Instances OnlyClass,FastVector head) throws Exception{
//		  int num=instances.numAttributes();
//		  int total=instances.numInstances();
//		  int numClass = OnlyClass.attribute(0).numValues();
...
//       	       }
  ...
  public FastVector mergeLabelItemSets(FastVector itemSets, int size, 
		    int totalTrans) {
...
if (k == first.m_items.length) {
	...
	else result.m_ruleSupCounter=second.m_ruleSupCounter;
result.m_counter = 0;
...
return newVector;
  ...
  private  void newMerge(RuleNode root,FastVector ksets,LinkedList<TNode> alpha,int total,int itemsize,int supsize,int min,double[] supB,int[] countc){
//		 m_numRules = 0;
...
//					    		 addRule(rule);
  ...
  private  void newMerge(RuleNodeII root,FastVector ksets,LinkedList<TNode> alpha,int total,int itemsize,int supsize,int min,double[] supB,int[] countc){
//		 m_numRules = 0;
		int numClass =  supsize;
...
//					    		 addRule(rule);
  ...
  private  void newMergeII(RuleNode ruleroot,FastVector ksets,LinkedList<TNode> alpha,int total,int itemsize,int supsize,int min,double[] supB,int[] countc){
...
//                 for (int k = 0;)
  ...
  private void mineCMARtree(CMARtree fp,FastVector head,LinkedList<TNode> alpha,int min,int max,RuleNode result,double[] supB,int[] countc) throws Exception{
...
private void mineCMARtree(CMARtree fp,FastVector head,LinkedList<TNode> alpha,int min,int max,RuleNodeII result,double[] supB,int[] countc) throws Exception{
...
//    				 addRule(rule);
 ...
 private void newmineCMARtree(CMARtree fp,FastVector head,LinkedList<TNode> alpha,int min,int max,RuleNode result,double[] supB,int[] countc) throws Exception{
...
//    				 m_Ls.addElement(rule);
 ...
 /* private FastVector mineProTree(FPtree fp,ListHead[] head,TNode a) throws Exception{
   ...
   if(t0.chnum==1){   
	  FastVector single=new FastVector();
	  TNode t=t0.child.get(0);
	  ItemSet is = new ItemSet(t,total);
	  single.addElement(is);
	  i++;     
	  while(t.chnum==1){ 
		  i++;
		  t=t.child.get(0);
		  is = new ItemSet(t,total);
		  single.addElement(is);
	  }
	  for(int j=0;j<t.chnum;j++){
		  TNode tc=t.child.get(j);		  
		  q0.addChild(tc);
	  }
	  t.chnum=0;
	  t.child=null;
	  ...
	  {
		  FastVector k1sets=new FastVector();
		 // k1sets=newTNodeMerge(ksets,j);
		  k1sets = AprioriItemSet.mergeAllItemSets(ksets, j, total);
		  plist.appendElements(k1sets);	
		  ksets=k1sets;
	  }
	   ksets = null;
   ...
   if(q0.chnum>0){
     for(int j = head.length-1; j >= i; j--){
		 ...
		 for(int l=0;l<lj.nextnum;l++){
		   ...
		   TNode t=tl0.father;
		   while(t.father!=null){
			   TNode rt=new TNode(t);
			   rt.m_counter = tl0.m_counter;
			   tl.addElement(rt);
			   t=t.father;
		   }
		   if(tl.size()>0){
	     ...
	     ArrayList<ListHead> CpTlist=new ArrayList<ListHead>();
		 FastVector t00=(FastVector)list.elementAt(0);
		 for(int l=0;l<t00.size();l++){
			 TNode temp0=(TNode)t00.elementAt(l);
		 ...
		 for(int l = 1;l < listsize; l++){
			 ...
			 for(int x=0;x<tl.size();x++){
				 TNode tempT=(TNode)tl.elementAt(x);
				 int flag=1;
				 for(int xx = 0;xx < CpTlist.size();xx++){
					 ListHead tempC=(ListHead)CpTlist.get(xx);
					 if(tempC.equal(tempT)){
						 tempC.count+=tempT.m_counter;
						 flag=0;
						 break;
					 }
				 }
				 if(flag == 1){
					 ListHead ln=new ListHead(tempT.m_counter,tempT.attr,tempT.value);
					 CpTlist.add(ln);
				 }
			 }
		 }
		 ...
		 ListHead[] cptHead = (ListHead[])CpTlist.toArray();
		 CpTlist = null;
		 if(listsize == 1){
			 FastVector ksets = new FastVector();
			 FastVector k_1sets=(FastVector)list.elementAt(0);
			 for(int xx = 0; xx < ksets.size(); xx++){
				 TNode node = (TNode)k_1sets.elementAt(xx);
				 ItemSet is = new ItemSet(node,total);
				 ksets.addElement(is);
			 }
			 subList.appendElements(ksets);
			 int deep = ksets.size()-1;
			 for(int x = 0; x < deep; x++){
				 FastVector k1sets=new FastVector();
				 k1sets = AprioriItemSet.mergeAllItemSets(ksets, x, total);
				 subList.appendElements(k1sets);
				 ksets=k1sets;
			 }
			 ksets = null;
		 ...
		 int CpTsize=cptHead.length;
		 
		 for(int x = 0;x < CpTsize; x++){ 
			 ListHead tempC=cptHead[x];
			 for(int xx=x+1;xx<CpTsize;xx++){
				 ListHead tempD=cptHead[xx];
				 if(tempD.count>tempC.count){
					 int tempa = tempC.attr;
					 int tempv = tempC.value;
					 int tempcount = tempC.count;
					 tempC.attr = tempD.attr;
					 tempC.value = tempD.value;
					 tempC.count = tempD.count;
					 tempD.attr = tempa;
					 tempD.value = tempv;
					 tempD.count = tempcount;
				 }
			 }
			 
		 }

		 FPtree subTree=buildCondFPtree(list,cptHead);	 
         if(subTree.root.chnum>0){
       	       subList=mineProTree(subTree,cptHead,b);      
          }
		 }
         b.mergeVector(subList);
         qlist.appendElements(subList);
         subList = null;
     }
   }
      ...
      for(int j=0;j<plist.size();j++){
    		 ItemSet pt=(ItemSet)plist.elementAt(j);
    		 for(int k=0;k<qlist.size();k++){
    			 ItemSet qt=(ItemSet)qlist.elementAt(k);
    			 ItemSet re=mergeItemSet(pt,qt);
    			 Result.addElement(re);
    		 }
      }
        plist = null;
  ...
  private FastVector newmineProTree(FPtree fp,ListHead[] head,TNode a,int min,int max) throws Exception{
   ...
   if(q0.chnum>0){
	   int sizenew = head.length;
     for(int j=sizenew - 1; j >= i; j--){ 	
    	 ListHead lj=head[j];
    	 TNode b=new TNode(lj.attr,lj.value);
    	 b.m_counter=lj.count;
    	 FastVector subList=new FastVector();
    	 FastVector list=new FastVector();
    	 setCarhashtable();
    	 int nextnumber = lj.nextnum;
		 for(int l = 0; l < nextnumber; l++){
		   FastVector tl=new FastVector(); 
	       FastVector nextList=lj.next;
		   TNode tl0=(TNode)nextList.elementAt(l);           ///t10 mining
		   TNode t=tl0.father;
		 ...
		 if(listsize == 1){
			 ...
			 Enumeration enu = m_carhashtable.keys();
			 FastVector ksets = new FastVector();
			 FastVector toDelete = new FastVector();
			 while(enu.hasMoreElements()){
				 ItemSet Is = (ItemSet)enu.nextElement();
				 int sup = ((Integer)m_carhashtable.get(Is)).intValue();
				 if(sup > necSupport){
					TNode node = new TNode(Is);
					node.m_counter = sup;
					ksets.addElement(node);
				 }
				 else{
					 AprioriItemSet Ais = new AprioriItemSet(m_instances.numInstances());
					 Ais.m_counter = sup;
					 Ais.m_items = Is.m_items;
					 toDelete.addElement(Ais);
				 }
			 }
			 m_carhashtable.clear();
			 if(ksets.size() == 0)
			 {
				 subList.addElement(b);
				 continue;
			 }
		     subList.appendElements(ksets);
		     ...
		     for(int k= 0; k < listsize; k++){        
		    	 ...
		    	 FastVector k_sets = (FastVector)list.elementAt(k);
		    	 int k_size = k_sets.size();
		    	 if(toDelete.size() > 0){
		    	   for (int kk = 0; kk < k_size; kk++){    
		    		TNode node = (TNode)k_sets.elementAt(kk);
		    		if(node.containedBy(toDelete))
		    			continue;
		    		else 
		    			ksets.addElement(node);
		    	   }
...
//			     int ksize = ksets.size();
				 ...
				 for (int x = 0; x < deep; x++){
					 FastVector k1sets=new FastVector();
					 k1sets=newTNodeMerge(ksets,x);
					 int k1size = k1sets.size();
					 for (int xx = 0; xx < k1size; xx++){
						 TNode tnode = (TNode)k1sets.elementAt(xx);
						 rehashTnode(tnode);
					 }
					 ksets=k1sets;
				 }
      ...
      Result.appendElements(qlist);
      for(int j=0;j<plist.size();j++){
    		 TNode pt=(TNode)plist.elementAt(j);
    		 for(int k=0;k<qlist.size();k++){
    			 TNode qt=(TNode)qlist.elementAt(k);
    			 TNode re=mergeTnode(pt,qt);
    			 Result.addElement(re);
    		 }
    	 }
  ...
  public void TnodeToItemSets(ArrayList<TNode> list) throws Exception{
	  int size=list.size();
	  ArrayList<TNode> ISlist=new ArrayList<TNode>();
	  FastVector ksets=new FastVector();
//	  FastVector mLs=new FastVector();
//	  FastVector hash=new FastVector();
	  Hashtable hashtable; 
	  for(int i=0;i<size;i++){
		  TNode node=list.get(i);
		  int lsize=ISlist.size();
		  if(lsize==0){
			  ISlist.add(node);
		  }
		  else{
	  ...
	  for(int i=0;i<k;i++){
		  TNode t=ISlist.get(i);
		  AprioriItemSet set=new AprioriItemSet(m_instances.numInstances());
		  set.m_counter=t.num;
		  set.m_items=t.value;
		  if(t.size()==kk){
			  ksets.addElement(set);
//			  displayItemset(set);
		  }
		  else{
			  m_text+="\n-----------------------"+kk+"\n";
			  m_Ls.addElement(ksets);
		      hashtable = AprioriItemSet.getHashtable(ksets, ksets.size());
		      m_hashtables.addElement(hashtable);
		      FastVector k1sets=new FastVector();
		      k1sets.addElement(set);
		      ksets=k1sets;
			  kk++;
		  }	 
      }
...
/*  private CMARtree newbuildCFPtree(FastVector instances,FastVector head) throws Exception{
	  int num=m_instances.numAttributes();
	  int size = instances.size();
	  int total=m_instances.numInstances();
	  int numClass = m_onlyClass.attribute(0).numValues();
...
//				  }
				  ...
				  if(t.chnum==0){
					...
					int flag=1;
					for(int jj=0;jj<t.chnum;jj++){
						TNode node = (TNode)t.child.elementAt(jj);
						if (node.m_items.equals(set.items)){
							flag=0;
							t=(TNode)t.child.elementAt(jj);
							t.m_counter += instance.m_counter;
							for(int k = 0; k < instance.m_sup.length; k++){
//								t.m_sup[k] = t.m_sup[k]+instance.m_sup[k];
								set.sup[k] = set.sup[k] + instance.m_sup[k];
							}
  ...
  private FPtree buildCondFPtree(FastVector list,ListHead[]CList) throws Exception{
	 // int numSets=m_instances.numAttributes();
	  FPtree fp=new FPtree();
	  int numInstance=list.size();
	  int size=CList.length;
	  for (int ii = 0; ii <numInstance;ii++) {
		  TNode t=fp.root;
		  FastVector tempList=(FastVector)list.elementAt(ii);
		  for(int j=0;j<size;j++){
			  ListHead set=CList[j];
			  if(set.containedBy(tempList)){
				  int count=((TNode)tempList.elementAt(0)).m_counter;
				  TNode tt=new TNode(set.attr,set.value);
				  tt.m_counter = count;
				  if(t.chnum==0){
					  t.addChild(tt);
  ...
  private CarFPtree buildCarCondFPtree(FastVector list,FastVector head) throws Exception{
	  int numSets=m_instances.numAttributes();
	  CarFPtree fp=new CarFPtree(numSets,m_instances.numInstances());
	  int numInstance=list.size();
	  int size=head.size();
	  for (int ii = 0; ii <numInstance;ii++) {
		  LabelTnode t = fp.root;
		  for(int j=0;j<size;j++){
			  ...
			  if(set.containedBy(tempList)){
				  LabeledItemSet Is=set.is.copy();
				  LabelTnode tt=new LabelTnode(Is);
				  int count=((LabelTnode)tempList.elementAt(0)).value.m_ruleSupCounter;
				  tt.value.m_ruleSupCounter=count;
				  if(t.chnum==0){
					  t.addChild(tt);
					  t=tt;
					  set.addNext(tt);
					...
					for(int jj=0;jj<t.chnum;jj++){
						if(t.child.get(jj).value.similar(Is)){
							flag=0;
							t=t.child.get(jj);
							t.value.m_ruleSupCounter+=count;
							break;
						}
					}
					if(flag>0){
...
//  }
 ...
 /* private FastVector newTNodeMerge(FastVector ksets,int size){
	  FastVector resultList=new FastVector();
	  //TNode result;
	    int numFound, k;
	    for (int i = 0; i < ksets.size(); i++) {
	    	TNode first = (TNode)ksets.elementAt(i);
	    	//m_text+="\n========"+i;
	    out:
	      for (int j = i+1; j < ksets.size(); j++) {
	    	//  m_text+="\t======="+j;
	    	TNode second = (TNode)ksets.elementAt(j);
	    	TNode result=new TNode();
	    	result.value=new int[first.value.length];
		numFound = 0;
		k = 0;
		while (numFound < size) {
		  if (first.value[k] == second.value[k]) {
		    if (first.value[k] != -1) 
		      numFound++;
		    result.value[k] = first.value[k];
		  } else 
		    break out;
		  k++;
		}
		// Check difference
       while (k < first.value.length) {
		  if ((first.value[k] != -1) && (second.value[k] != -1))
		    break;
		  else {
		    if (first.value[k] != -1){
		      result.value[k] = first.value[k];
		...
		if (k == first.value.length) {
		  if(first.num < second.num){
			  result.num=first.num;
		  }
		  else{
			  result.num=second.num;
		  }
 ...
 */
 public FastVector buildClassifyNorules(Instances instances,Instances onlyClass,double min,double max,double minMetric,double minConv)throws Exception{
 ...
 private  void newMerge(FastVector ksets,int sizealpha,int total,int itemsize,int supsize,int min,double[] supB,double[] dPro){
 ...
 private  void newMergeII(FastVector ksets,int sizealpha,int total,int itemsize,int supsize,int min,double[] supB,double[] dPro){
 ...
 private void mineCMARtree(CMARtree fp,FastVector head,LinkedList<TNode> alpha,int min,int max,double[] supB,double[] dPro) throws Exception{
  ...
  public static void main(String[] args){
	  //args={"-t zoo.arff "};
	//  int a=1;
	  String[] arg={"-t","zoo.arff","-M","0.02","-A","-C","0"};
      runAssociator(new FP_backup(),arg);
//	  for (int i = 0; i < 10;i++){
